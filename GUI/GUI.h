...
private:
	...
	static const int
		WindWidth = 1080, WindHeight = 650,		//Window width and height
...
public:
	...
	// Output Functions  ---------------------------
	void PrintMessage(REGION kind, string msg, int x, color c = BLACK) const; // prints a message in the status bar
	void PrintMessage(string msg, int x = 10, int y = WindHeight - (int)(StatusBarHeight / 1.5)) const;
	...
	void DrawCastle(int disA, int disB, int disC, int disD) const;	    // draws the castle with the towers and 4 regions
